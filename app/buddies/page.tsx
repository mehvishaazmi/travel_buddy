...
import {
  Search, MapPin, Calendar as CalendarIcon, Sparkles, BadgeCheck,
  Heart, MessageCircle, Send, Filter, Wallet, X, Check, Loader2,
} from "lucide-react";
...
const ALL_INTERESTS = [
  "Hiking","Photography","Foodie","Surfing","Yoga","Coffee",
  "Art","Nightlife","Slow travel","Backpacking","Diving","Local markets",
];
...
export default function Buddies() {
  ...
  async function fetchTripRequests() {
    ...
    (data ?? []).forEach((r: any) => { map[r.trip_id] = r.status; });
    setTripRequests(map);
  }
  ...
  async function toggleSave(buddy: BuddyWithTrip, e: React.MouseEvent) {
    ...
    if (isSaved) {
      ...
      setSavedIds((prev) => { const s = new Set(prev); s.delete(buddy.user_id); return s; });
    } else {
      await supabase
        .from("saved_buddies")
  ...
  async function handleTripRequest(action: "accepted" | "declined", tripId: string) {
  ...
  const filtered = useMemo(() => {
    return buddies.filter((b) => {
      if (destination && !b.trip?.destination.toLowerCase().includes(destination.toLowerCase()))
        return false;
      if (b.trip && budget[0] < 15000 && Number(b.trip.budget) > budget[0]) return false;
    ...
    <div className="space-y-7">
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-semibold text-primary">up to ₹{budget[0].toLocaleString()}</span>
        </div>
        <div className="mt-3 flex items-center gap-3">
          <Wallet className="h-4 w-4 text-muted-foreground shrink-0" />
          <Slider value={budget} onValueChange={setBudget} min={500} max={15000} step={500} />
        </div>
      </div>
      ...
      <div>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className={cn("w-full mt-2 h-11 rounded-xl justify-start text-left font-normal",
      ...
      {(destination || interests.length > 0 || budget[0] !== 3000) && (
        <Button variant="ghost" className="w-full rounded-xl text-muted-foreground"
          onClick={() => { setDestination(""); setInterests([]); setBudget([5000]); setDateRange(undefined); }}>
      ...
      <section className="pb-24">
        <div className="container grid lg:grid-cols-[280px_1fr_360px] gap-6">
          ...
          <div className="lg:hidden flex items-center justify-between">
            <span className="text-sm text-muted-foreground">{filtered.length} buddies found</span>
            <Sheet>
          ...
          <div className="grid sm:grid-cols-2 gap-5 content-start">
            ...
            {filtered.map((b) => {
              ...
              return (
                <article key={b.id} onClick={() => { setActiveBuddy(b); fetchMessages(b.user_id); }}
                  className={cn(
                    "group cursor-pointer rounded-3xl bg-card border p-6 shadow-soft hover:shadow-card hover:-translate-y-1 transition-smooth",
                    isActive ? "border-primary/60 shadow-card ring-2 ring-primary/20" : "border-border/60"
                  )}
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="text-right">
                      <div className="font-display text-2xl font-bold text-gradient leading-none">{b.match}%</div>
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mt-1">match</div>
                    </div>
                  </div>

                  <p className="mt-4 text-sm text-foreground/75 leading-relaxed line-clamp-2">{b.bio}</p>

                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {(b.interests ?? []).map((tag) => (
                      <span key={tag} className="px-2.5 py-1 rounded-full bg-secondary text-[11px] font-medium text-foreground/70">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-5 pt-5 border-t border-border/60 flex items-center justify-between gap-3">
                    <div className="text-xs text-muted-foreground min-w-0">
                      {b.trip ? (
                        <>Heading to <span className="font-semibold text-foreground">{b.trip.destination}</span></>
                      ) : (
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <Button size="sm" variant="hero" className="rounded-xl h-9"
                        onClick={(e) => { e.stopPropagation(); setActiveBuddy(b); fetchMessages(b.user_id); }}>
                        Connect
                      </Button>
                    </div>
                  </div>
                  ...
                  {tripStatus && (
                    <div className={cn(
                      ...
                      tripStatus === "accepted" ? "bg-green-500/10 text-green-500" :
                      tripStatus === "declined" ? "bg-red-500/10 text-red-500" :
          ...
          {activeBuddy && (
            <aside className="lg:sticky lg:top-28 self-start">
              <div className="rounded-3xl border border-border/60 bg-card shadow-elevated overflow-hidden">
                ...
                <div className="p-5 border-b border-border/60 flex items-center gap-3 gradient-soft">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-display font-semibold flex items-center gap-1.5">
                      {activeBuddy.name}
                      {activeBuddy.is_verified && <BadgeCheck className="h-4 w-4 text-primary" />}
                    </div>
                    ...
                    <div className="px-5 pb-4">
                      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4">
                        ...
                        <div className="mt-1.5 text-sm font-semibold">
                          Join {activeBuddy.name.split(" ")[0]}'s trip to {activeBuddy.trip.destination}
                        </div>
                        <div className="text-xs text-muted-foreground mt-0.5">
                          {activeBuddy.trip.days} days · up to ₹{Number(activeBuddy.trip.budget).toLocaleString()}
                        </div>
                        {status === "accepted" ? (
                          <div className="mt-3 text-sm font-semibold text-green-500 flex items-center gap-1.5">
                            <Check className="h-4 w-4" /> You joined this trip!
                          </div>
                        ) : status === "declined" ? (
                          <div className="mt-3 text-sm text-muted-foreground">Request declined.</div>
                        ) : (
                          <div className="mt-3 flex gap-2">
                            <Button size="sm" variant="hero" className="flex-1 rounded-lg"
                              onClick={() => handleTripRequest("accepted", activeBuddy.trip!.id)}>
                              <Check className="h-4 w-4" /> Accept
                            </Button>
                            <Button size="sm" variant="outline" className="rounded-lg"
                              onClick={() => handleTripRequest("declined", activeBuddy.trip!.id)}>
                              Decline
                            </Button>
                          </div>
                        )}
                ...
                {/* Chat input */}
                <form onSubmit={sendMessage} className="p-3 border-t border-border/60 flex gap-2 bg-card">
                  <Input
                    value={chatInput}
                  ...
                  <Button type="submit" variant="hero" size="icon"
                    className="h-11 w-11 rounded-xl shrink-0" disabled={sendingMsg}>
